...
class GetMRI(Dataset):
    ...
    def __getitem__(self,index):
        data = loadmat(self.data_names[index])['data']
        data_stack = np.stack([np.real(data), np.imag(data)], axis=0)
        input_data = np.concatenate([data_stack, data_stack, data_stack], 0)
        return input_data
...
def get_dataset(config, uniform_dequantization=False, evaluation=False):
  ...
  dataset = GetMRI(root= "./input_datasets/input_data/data_train")
  test_dataset = GetMRI(root= "./input_datasets/input_data/data_eval")
